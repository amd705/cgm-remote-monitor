...
function configure (app, wares, ctx, env) {
  ...
  function config_authed (app, api, wares, ctx) {
    ...
    function post_response (req, res) {
      ...
      console.log('config_authed / post_response / treatments')
      ...
      ctx.treatments.create(treatments, function(err, created) {
        if (err) {
          console.log('ctx.treatments.create - Error adding treatment!!!', err);
          res.sendJSONStatus(res, constants.HTTP_INTERNAL_ERROR, 'Mongo Error', err);
        } else {
          console.log('REST API treatment created');
          // res.json(created);
          res.json({});
        }
      });
    ...
    // update record
    api.put('/treatments/',  wares.bodyParser({
      limit: '50mb'
    }), ctx.authorization.isPermitted('api:treatments:update'), function(req, res) {
      console.log('api.put.treatments');
      var data = req.body;
